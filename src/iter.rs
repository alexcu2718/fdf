...
use libc::DIR;
use crate::tempdirentry::TempDirEntry;
...
pub trait DirentConstructor {
    ...
    fn get_fd(&self)->i32; //TODO COMMENT LATER
    ...
    unsafe fn construct_entry(&mut self, drnt: *const dirent64) -> TempDirEntry<'_> {
            ...
            FileType::Unknown => {
                ...
                let cstr_name: &CStr = unsafe {
                    CStr::from_bytes_with_nul_unchecked(
                        full_path.to_bytes_with_nul().get_unchecked(base_len..),
                    )
                };
                FileType::from_fd_no_follow(fd, cstr_name)
            }
    ...
    ($struct:ty) => {
        impl $struct {
            ...
            */
            ...
            ) -> $crate::tempdirentry::TempDirEntry<'_> {
        ...
        impl Iterator for $type {
            ...
            fn next(&mut self) -> Option<Self::Item> {
                while let Some(drnt) = self.get_next_entry() {
                    ...
                    let temp_direntry=self.construct_direntry(drnt);
                    return Some(temp_direntry.to_owned());
                }
