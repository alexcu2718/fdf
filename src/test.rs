...
mod tests {
    ...
    fn testmemchr(search: u8, sl: &[u8]) {
        let memchrtest = crate::memchr(search, sl);
    ...
    fn test_memrchr(search: u8, sl: &[u8]) {
        ...
        assert!(
            memrchrtest == realans,
            "test failed for {} in memrchr real ans is {realans:?} {memrchrtest:?} for character {char}",
            String::from_utf8_lossy(sl)
        );
    }

    #[test]
    fn test_charactersearch() {
        let random_chars = 0..u8::MAX;

        for items in random_chars {
            for paths in PATHS {
                testmemchr(items, *paths);
