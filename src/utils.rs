...
pub fn strlen<T>(ptr: *const T) -> usize
where
    T: ValueType,
{
    crate::strlen_asm!(ptr)
...
/// Returns -1 on error.
pub fn open_asm(bytepath: &[u8]) -> i32 {
...
pub fn open_asm(bytepath: &[u8]) -> i32 {
    ...
    libc::open(cstr!(bytepath), libc::O_CLOEXEC | libc::O_DIRECTORY | libc::O_NONBLOCK | libc::O_RDONLY) 
}
