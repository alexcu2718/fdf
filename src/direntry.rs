...
impl DirEntry {
    ...
    */
    ...
    /// Checks if a symlink points to a traversible directory, caching the result (similar to above but using fstatat)
    #[inline]
    pub(crate) fn check_symlink_traversibility_at(&self,fd:&FileDes) -> bool {
        debug_assert!(
            self.file_type() == FileType::Symlink,
            "we only expect symlinks to use this function(hence private)"
        );
