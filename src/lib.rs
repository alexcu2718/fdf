...
impl Finder {
    ...
    */
    ...
    fn should_traverse(&self, dir: &DirEntry,fd:&FileDes) -> bool {
        match dir.file_type {
            ...
            FileType::Symlink if self.search_config.follow_symlinks => {
                dir.check_symlink_traversibility_at(fd)
            }
    ...
    */
    fn process_directory(&self, dir: DirEntry, sender: &Sender<Vec<DirEntry>>) {
        ...
        match direntries {
            Ok(entries) => {
                let dirfd=entries.dirfd().clone();
                let (dirs, mut files): (Vec<_>, Vec<_>) = entries
                    .filter(|entry| {
                        self.keep_hidden(entry)
                            && (self.should_traverse(entry,&dirfd) || self.file_filter(entry))
                    })
                    .partition(|ent| self.should_traverse(ent,&dirfd));
